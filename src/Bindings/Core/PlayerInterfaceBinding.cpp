#include "pch.h"
<<<<<<< HEAD:src/Bindings/Core/PlayerInterfaceBinding.cpp
...
>>>>>>> main:src/Bindings/PlayerInterfaceBinding.cpp
...
<<<<<<< HEAD:src/Bindings/Core/PlayerInterfaceBinding.cpp
        ...
        if (strcmp(key, "selectedCharacters") == 0) {
            lua_createtable(L, static_cast<int>(p->selectedCharacters.size()), 0);
            int i = 1;
...
<<<<<<< HEAD:src/Bindings/Core/PlayerInterfaceBinding.cpp
    int PlayerInterfaceBinding::setCharacterEditMode(lua_State* L)
    {
        PlayerInterface* p = getP(L, 1);
        if (!p) return luaL_error(L, "PlayerInterface is nil");
        bool editor = lua_toboolean(L, 2) != 0;
