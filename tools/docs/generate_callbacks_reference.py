...
Usage:
    python tools/docs/generate_callbacks_reference.py
"""
...
HOOKS_REGISTRY_CPP = PROJECT_ROOT / "src" / "Hooks" / "Hooks_Registry.cpp"
...
def parse_hooks_registry():
    """Extract eventName -> installer map from g_eventHookRegistry in Hooks_Registry.cpp."""
    event_map = {}
    if not HOOKS_REGISTRY_CPP.exists():
        return event_map

    content = HOOKS_REGISTRY_CPP.read_text(encoding="utf-8", errors="ignore")
...
def generate_markdown(callbacks, registry_map):
    ...
    output.append("")
    output.append("This document is automatically generated by `tools/docs/generate_callbacks_reference.py` based on `src/Callbacks.h` and `src/Hooks/Hooks_Registry.cpp`.")
    output.append("")
